...
// AeccWin32Utils.h includes the system header intrin.h via AeccErrorMessage.h
// Note: If we don't include math.h before AeccWin32Utils.h, then we get the following compile error on x64:
// 1>C:\Program Files (x86)\Microsoft Visual Studio 9.0\VC\include\math.h(136) : error C2220: warning treated as error - no 'object' file generated
// 1>C:\Program Files (x86)\Microsoft Visual Studio 9.0\VC\include\math.h(136) : warning C4985: 'ceil': attributes not present on previous declaration.
// 1>        C:\Program Files (x86)\Microsoft Visual Studio 9.0\VC\include\intrin.h(142) : see declaration of 'ceil'
// This happens because math.h declares ceil as _CRTIMP double  __cdecl ceil(_In_ double _X);
// while intrin.h declares it as _CRTIMP double  ceil(_In_ double _X);
// Note the missing __cdecl - this works on x86 because __cdecl is the default calling convention
// However, on x64, the default calling convention is not __cdecl. This appears to be a defect with intrin.h
#include <math.h>
//#include "AeccWin32Utils.h"
...
#pragma warning( disable : 4324 )				// warning C4324: 'AeccDetails::FreeListBlockHeader' : structure was padded due to __declspec(align())
...
void *const &AeccFreeListDebug_FillMemory(void *const &ptr,const size_t &size,const unsigned char &val)
{
...
namespace AeccDetails
{
	...
	// these pages are used to supply the AeccFreeList with its pages
	class PageFreeList
	{
...
*/
		...
		void *AllocateBlock(const size_t &blockSize=BLOCK_SIZE)
		{
			...
			else
			{
                ...
                ASSERT(0);
				//int nRet = AeccWin32Utils::CallAcadNewHandler( blockSize );
				//if( nRet != 0 )
				//	ptr = AllocateBlock(blockSize);		// if CallAcadNewHandler returns non-zero, then it released the pre-allocated low-mem block - retry the allocation
				//else									// if VirtuaAlloc fails for a second time, we'll come in here again and m_pAcadNewHandler will throw an exception.
				//	__assume(0);						// since m_pAcadNewHandler throws an exception on failure, we'll never get here
			}
			return ptr;
		}
	...
	// be sure and update the AECC_FREELIST_PAGE_SIZE macro
...
#pragma pop_macro("new")
	...
	size_t AECCNOALIAS FreeList::ListLength(const ListEntry *m_pListHead)
	{
		size_t nSize = 0;
...
//	HANDLE PageFreeList::m_new_handler_mutex = NULL;
	...
	size_t PageFreeList::WalkPages()
	{
		...
		__print(_T(".\n"));
		__print(_T("AeccFreeList<T>: Currently a total of %Iu pages have been allocated by VirtualAlloc() in %Iu 64k blocks corresponding to %Iu bytes\n"), sTotalSpaceAllocated/PAGE_SIZE, sTotalSpaceAllocated/BLOCK_SIZE, sTotalSpaceAllocated);
		__print(_T("AeccFreeList<T>: of these pages a total of %Iu pages are in use by the AeccFreeList<T> allocators while %Iu unused pages are on the PageFreeList list\n"), sTotalPagesInUse, sTotalPagesOnPageFreeList);
		__print(_T(".\n"));
			...
			++it )
		{
			...
			if( jt != theList.end() )
			{
				...
				if( pHeader && pHeader->NumItems() > 0 )
				{
					...
					if( bIsPageBlockList )
					{
						__print(_T("AeccFreeList<T>: Have %d pages devoted to block size %d with %d blocks per page and %d bytes wasted at the end of the page. "), 
							nPages, 
							(DWORD)itemSize, 
							dwItemsPerPage, 
							dwWastedPageSpace );
					...
					{
						__print(_T("AeccFreeList<T>: Have a 16 page allocation in a Large Block Free List devoted to block size %d with %d blocks in the allocation and %d wasted space. "),
							(DWORD)itemSize, 
							dwItemsPerPage, 
							dwWastedPageSpace );
					}
		...
		__print(_T(".\n"));
		__print(_T("Total Pages allocated by AeccFreeList<T> %d\n"), static_cast<DWORD>(nTotalPagesInUse));
		__print(_T("Total Space allocated by AeccFreeList<T> currently in use %d bytes\n"), static_cast<DWORD>(nTotalSpaceInUse));
		__print(_T("Total Space allocated by AeccFreeList<T> currently not in use %d bytes\n"), static_cast<DWORD>(nTotalSpaceFree));
		__print(_T(".\n"));
...
extern "C"
{
	...
	__declspec(dllexport)
	int
	AeccFreeListReportPagesInUse()
	{
		return AeccDetails::PageFreeList::m_nPagesInUse;
	}
	__declspec(dllexport)
	int
	AeccFreeListReportPageBlocksInUse()
	{
		return AeccDetails::PageFreeList::m_nPageBlocksInUse;
	}
	__declspec(dllexport)
	void
	AeccFreeListWalkFreeLists()
	{
		AeccDetails::FreeListBlockHeader::DoWalkPages();
		AeccDetails::FreeList::WalkFreeLists();
	}
}
#endif // AECC_NONPROD
